...
namespace PostgresApp1.Queries.Actors
{
    public class ActorQuery
    {
        public object ActorId { get; private set; }

        public Actor Execute()
        {
            var sql = @"select first_name as first, last_name as last,
                        actor_id as ID from actor where actor_id=@0;";
